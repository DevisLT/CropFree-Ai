...
export default function AICoach() {
  ...
  return (
    <div className="max-w-6xl mx-auto h-[calc(100vh-120px)] md:h-[calc(100vh-180px)] flex flex-col premium-card rounded-[48px] shadow-[0_0_100px_rgba(0,0,0,0.5)] overflow-hidden border-white/5 relative group/shell font-sans text-left">
      {/* Background Ambience */}
         ...
         <div className="flex items-center gap-4 md:gap-8">
            ...
            <div>
               <div className="flex items-center gap-3 mb-1">
                 <h3 className="font-black text-xl md:text-3xl tracking-tighter text-white">{t('neural_coach') || "Neural Coach."}</h3>
               ...
               <div className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em]">{t('encrypted_connection') || "Encrypted Connection Active"}</span>
               </div>
            </div>
         </div>
         <div className="flex items-center gap-4">
            <button type="button" className="hidden sm:flex px-6 py-3 bg-white/5 hover:bg-white/10 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] transition-all border border-white/5 text-slate-400 hover:text-white items-center gap-3">
               <Terminal className="w-4 h-4" /> {t('export_log') || "Export LOG"}
            </button>
            <button type="button" className="w-12 h-12 md:w-14 md:h-14 bg-rose-500/10 text-rose-500 hover:bg-rose-500 hover:text-white rounded-2xl transition-all flex items-center justify-center border border-rose-500/10 active:scale-90">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 md:p-14 space-y-10 md:space-y-16 scroll-smooth custom-scrollbar bg-slate-950/20">
         {messages.map((msg, i) => (
           <motion.div 
             ...
             className={`flex gap-6 md:gap-10 ${msg.role === 'user' ? (isRTL ? 'flex-row' : 'flex-row-reverse') : (isRTL ? 'flex-row-reverse' : '')}`}
           >
             ...
             <div className={`max-w-[85%] md:max-w-[70%] p-8 md:p-12 rounded-[32px] text-base md:text-xl font-bold leading-relaxed shadow-xl relative border text-left ${
               msg.role === 'user' 
                 ? 'bg-slate-900/90 text-white border-white/5' 
                ...
                {msg.role === 'model' && (
                  <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                       ...
                       <span className="text-[10px] font-black text-slate-600 uppercase tracking-[0.4em]">{t('response_verified') || "Neural Response Verified"}</span>
                    </div>
         ...
         {isTyping && (
           <div className={`flex gap-6 md:gap-10 ${isRTL ? 'flex-row-reverse' : ''}`}>
              ...
              <div className="bg-white/5 p-6 md:p-10 rounded-[32px] flex items-center gap-4 border border-white/5 backdrop-blur-2xl">
                 ...
                 <span className="ml-4 text-[11px] font-black text-slate-600 uppercase tracking-[0.5em]">{t('synthesizing_thought') || "Synthesizing thought..."}</span>
              </div>
           </div>
         )}
      </div>
      ...
      <div className="p-8 md:p-16 bg-slate-950/40 backdrop-blur-3xl border-t border-white/5">
         <div className="max-w-5xl mx-auto relative group">
            ...
            <input 
              ...
              placeholder={t("ask_anything") || t("ask_coach_placeholder") || "Query Neural Interface..."}
              className={`w-full bg-white/5 border border-white/10 rounded-[32px] ${isRTL ? 'pr-16 md:pr-24 pl-32 md:pl-48' : 'pl-16 md:pl-24 pr-32 md:pr-48'} py-8 md:py-10 focus:outline-none focus:border-brand/40 focus:bg-white/10 transition-all text-lg md:text-2xl font-black text-white placeholder:text-slate-805 shadow-2xl uppercase tracking-tighter`}
            />
            <div className={`absolute ${isRTL ? 'right-6 md:right-10' : 'left-6 md:left-10'} top-1/2 -translate-y-1/2 text-slate-605 group-focus-within:text-brand transition-colors`}>
            ...
            <div className={`absolute ${isRTL ? 'left-6 md:left-10' : 'right-6 md:right-10'} top-1/2 -translate-y-1/2 flex items-center gap-4`}>
               <button type="button" className="hidden sm:block p-4 text-slate-700 hover:text-brand transition-all active:scale-90">
         ...
         <div className="flex items-center justify-center gap-4 mt-10">
            ...
            <p className="text-[10px] font-black text-slate-700 uppercase tracking-[0.5em]">{t('system_architecture') || "System Architecture: Quantum Neural Net v9.4"}</p>
            <div className="h-[1px] w-12 bg-white/5" />
         </div>
      </div>
    </div>
